...
struct BoldMovesLiveHereWebViewBox: UIViewControllerRepresentable {
  // MARK: - Свойства

  @ObservedObject var loader: BoldMovesLiveHereWebLoader
  var defaultOrientations: UIInterfaceOrientationMask = .landscape

  // MARK: - Координатор

  func makeCoordinator() -> BoldMovesLiveHereWebCoordinator {
    BoldMovesLiveHereWebCoordinator { [weak loader] status in
  ...
  func makeUIViewController(context: Context) -> BoldMovesLiveHereBaseWebViewController {
    let configuration = createWebViewConfiguration()
    return BoldMovesLiveHereBaseWebViewController(
  ...
  func updateUIViewController(_ uiViewController: BoldMovesLiveHereBaseWebViewController, context: Context) {
