...
protocol WebLoadable: AnyObject {
  var state: BoldMovesLiveHereWebStatus { get set }
...
final class BoldMovesLiveHereWebLoader: NSObject, ObservableObject, WebLoadable, ProgressMonitoring {
  // MARK: - Свойства

  @Published var state: BoldMovesLiveHereWebStatus = .standby
  ...
  init(resourceURL: URL) {
    ...
    if let cached = FCMManager.cachedToken() {
      self.resource = BoldMovesLiveHereWebLoader.makeResourceURL(baseURL: resourceURL, token: cached)
    } else {
  ...
  func observeProgression() {
    progressPublisher
      ...
      .sink { [weak self] progress in
        ...
        let newState: BoldMovesLiveHereWebStatus =
  ...
  private func applyFCMToken(_ token: String) {
    let newURL = BoldMovesLiveHereWebLoader.makeResourceURL(baseURL: baseResource, token: token)
...
extension BoldMovesLiveHereWebLoader: WKNavigationDelegate {
...
extension BoldMovesLiveHereWebLoader {
  ...
  func publish(_ newState: BoldMovesLiveHereWebStatus) {
...
extension BoldMovesLiveHereWebLoader {
  fileprivate func updateState(_ newState: BoldMovesLiveHereWebStatus) {
